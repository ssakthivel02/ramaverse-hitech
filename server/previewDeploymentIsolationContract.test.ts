...
describe("RamaVerse preview deployment isolation contract", () => {
  it("requires a dedicated RamaVerse database service, not only a dedicated database name", () => {
    ...
    expect(plan).toContain("separately provisioned MySQL or MySQL-compatible managed service dedicated to RamaVerse preview");
  ...
  it("binds preview schema setup to an explicit dedicated host identity without provider lock-in", () => {
    ...
    expect(workflow).not.toContain("actualHost.endsWith('.aivencloud.com')");
    expect(workflow).not.toContain("DATABASE_EXPECTED_SERVICE_MARKER");
  ...
  it("keeps schema setup on verified TLS for public and custom CA providers", () => {
    const workflow = read(".github/workflows/preview-db-setup.yml");
    ...
    expect(drizzle).toContain('process.env.DATABASE_REQUIRE_TLS === "true"');
    expect(drizzle).toContain("rejectUnauthorized: true");
    expect(drizzle).toContain('minVersion: "TLSv1.2"');
  });
  ...
  it("documents all secrets required by the fail-closed DB setup gate", () => {
    ...
    expect(plan).toContain("RAMAVERSE_PREVIEW_DATABASE_CA_CERT_B64");
    expect(plan).toContain("known shared `hitech-preview-mysql` service is explicitly rejected");
    expect(plan).toContain("public-CA providers use the system trust store");
  });
