...
const blockers = fs.readFileSync(path.resolve(import.meta.dirname, "../lib/finalBlockers.ts"), "utf8");
...
describe("Owner Command Center contract", () => {
  ...
  it("contains the requested state sections", () => {
    for (const id of ["health", "corpus", "content", "languages", "media", "parity", "release", "quality", "blockers", "actions"]) {
      expect(source).toContain(`id=\"${id}\"`);
    }
  });

  it("shows exact blocker evidence and keeps automatic resolution disabled", () => {
    expect(source).toContain('title="Exact unblock actions"');
    expect(source).toContain("finalBlockers.map");
    expect(blockers).toContain("automaticResolutionPossible");
    expect(blockers).toContain("ownerActionRequired");
    expect(blockers.match(/requirementId/g)?.length).toBeGreaterThanOrEqual(38);
  });
