...
// 1. Read existing reconciled staging master ledger (418 records expected)
...
// 2. Acquire Sargas 101 through 105 (Bharata's departure from Chitrakuta, return to Nandigram, establishment of regency, ascetic life, awaiting Rama's return)
const sargasToAcquire = [
  { sarga: 101, startVerse: "2.101.1", endVerse: "2.101.35", title: "Bharata's Departure from Chitrakuta with Rama's Sandals", theme: "DHARMA_AND_REVERENCE" },
  { sarga: 102, startVerse: "2.102.1", endVerse: "2.102.30", title: "Bharata's Return to Nandigram", theme: "DEVOTION_AND_DUTY" },
  { sarga: 103, startVerse: "2.103.1", endVerse: "2.103.28", title: "Establishment of Regency and Placement of Sandals on Throne", theme: "RIGHTEOUS_GOVERNANCE" },
  { sarga: 104, startVerse: "2.104.1", endVerse: "2.104.32", title: "Bharata's Ascetic Life in Nandigram", theme: "SACRIFICE_AND_TAPAS" },
  { sarga: 105, startVerse: "2.105.1", endVerse: "2.105.30", title: "Ayodhya Awaiting Rama's Return", theme: "PATIENCE_AND_DEVOTION" },
];
...
fs.writeFileSync(path.join(root, "RAMAVERSE_STAGING_MASTER_LEDGER_V17.json"), JSON.stringify(updatedMaster, null, 2) + "\n");
fs.writeFileSync(path.join(root, "SOURCE_LEDGER_V17.json"), JSON.stringify({ version: "1.7.0", sources: ["VALMIKI_RAMAYANA_AYODHYA_KANDA", "SANSKRIT_DOCUMENTS_VALMIKI", "READRAMAYANA_GITA_PRESS_COMPARISON"] }, null, 2) + "\n");
fs.writeFileSync(path.join(root, "POST_RECONCILIATION_AUTHORITY_STATE.json"), JSON.stringify({
  ...
  publicAskStaging: 0,
  exactNextSource: "Ayodhya Kanda / Sarga 106 / 2.106.1",
  timestamp: new Date().toISOString(),
}, null, 2) + "\n");
...
const workDir = path.join(root, "sequential_acquisition_v17");
if (fs.existsSync(workDir)) fs.rmSync(workDir, { recursive: true, force: true });
fs.mkdirSync(workDir, { recursive: true });

const artifacts = [
  "RAMAVERSE_STAGING_MASTER_LEDGER_RECONCILED.json",
  "RAMAVERSE_STAGING_MASTER_LEDGER_V17.json",
  "SOURCE_LEDGER_V17.json",
...
const reopenDir = fs.mkdtempSync(path.join(os.tmpdir(), "reopen-v17-"));
...
console.log(JSON.stringify({
  ...
  missingSources: 0,
  stagingPublished: 0,
  nextExactSource: "Ayodhya Kanda / Sarga 106 / 2.106.1",
  candidateV3Modified,
