...
console.log(`Validation V14: Physical count = ${physicalCount}, Ledger count = ${masterLedger.physicallyAvailableUniqueRecords}`);
...
fs.writeFileSync(path.join(root, 'VALIDATION_SEQUENTIAL_V14.json'), JSON.stringify(report, null, 2));

if (!match) {
  console.error("V14 Validation FAILED!");
  process.exit(1);
} else {
  console.log("V14 Validation PASSED successfully!");
}
