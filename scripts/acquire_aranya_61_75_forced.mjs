...
const newRecords = specs.map((s) => {
  const recordId = `STAGING-ARANYA-SARGA-${s.sarga}-FORCED-2026`;
  return {
    recordId,
    id: recordId,
    kanda: "Aranya Kanda",
...
const updated = {
  version: "2.6.0-aranya-61-75-forced",
...
fs.writeFileSync(path.join(root, "RAMAVERSE_STAGING_MASTER_LEDGER_V26.json"), JSON.stringify(updated, null, 2) + "\n");
...
// Package ZIP
const work = path.join(root, "aranya_61_75_forced_work");
if (fs.existsSync(work)) fs.rmSync(work, { recursive: true, force: true });
fs.mkdirSync(work, { recursive: true });

const artifacts = [
  "RAMAVERSE_STAGING_MASTER_LEDGER_RECONCILED.json",
  "RAMAVERSE_STAGING_MASTER_LEDGER_V26.json",
...
const reopen = fs.mkdtempSync(path.join(os.tmpdir(), "aranya-61-75-forced-"));
