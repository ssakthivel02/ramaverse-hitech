...
const batches = [
  "AYODHYA_S21_SOURCE_BACKED_RECORDS.json",
  "AYODHYA_S22_SOURCE_BACKED_RECORDS.json",
  "AYODHYA_S23_SOURCE_BACKED_RECORDS.json",
...
const editorialIndex = {
  ...
  entries: records.map((record) => ({
    ...
    contentSafety: record.content_safety ?? "STANDARD_SOURCE_REVIEW",
...
const graph = {
  ...
  edges: [
    ...
    { from: "stg-postv1-ayodhyakanda-s23-theme-001", to: "stg-postv1-ayodhyakanda-s22-theme-001", relation: "contrasts_with" }
  ],
};

fs.writeFileSync(path.join(root, "POST_V1_EDITORIAL_INDEX_V4.json"), JSON.stringify(editorialIndex, null, 2) + "\n");
fs.writeFileSync(path.join(root, "POST_V1_STAGING_KNOWLEDGE_GRAPH_V4.json"), JSON.stringify(graph, null, 2) + "\n");
