...
const groundedAnswer = {
  answer: "The canonical corpus connects this question to Hanuman. A canonical character record.",
  evidence: [
    {
      recordId: "CHR-4",
      type: "character",
  ...
  relatedQuestions: ["What is the source locator for this record?"],
  characters: [
    {
      recordId: "CHR-4",
      type: "character",
  ...
  provenance: {
    ...
    recordIds: ["CHR-4"],
    sourceLocators: ["Valmiki Ramayana Kishkindha Kanda"],
    confidence: "high",
  },
      ...
      intelligenceAsk: {
        useQuery: (_input: unknown, options?: { enabled?: boolean }) =>
          options?.enabled === false
            ? { isFetching: false, data: undefined, error: null }
            : { isFetching: false, data: groundedAnswer, error: null },
      },
    },
  },
}));
...
describe("Intelligence workbench", () => {
  it("shows grounded evidence and provenance fields", () => {
    ...
    render(<Intelligence />);
    fireEvent.change(screen.getByRole("textbox", { name: /grounded question/i }), {
      target: { value: "Hanuman" },
    });
  ...
  it("surfaces an explicit status when an offline question has no cached answer", () => {
    ...
    render(<Intelligence />);
    fireEvent.change(screen.getByRole("textbox", { name: /grounded question/i }), {
      target: { value: "Uncached question" },
    });
    fireEvent.submit(screen.getByRole("textbox", { name: /grounded question/i }).closest("form")!);
