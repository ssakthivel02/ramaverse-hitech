...
import { localizedPath, useTranslation, type SupportedLanguage } from "@/contexts/MultilingualContext";
      ...
      <main className="mx-auto max-w-7xl px-4 pb-20 pt-12 sm:px-6 lg:px-8">
        <section className="rv-experience-hero mb-8 overflow-hidden rounded-[2rem] border border-[#d7b45a]/25 px-6 py-10 sm:px-10">
          <div className="relative z-10 max-w-3xl">
            ...
            <div className="mt-7 flex flex-wrap gap-3"><Link href={localizedPath(language, "/walk-with-rama")}><Button className="bg-[#d7b45a] text-[#0b101a] hover:bg-[#f4d98b]">Walk with Rama</Button></Link><Link href={localizedPath(language, "/search")}><Button variant="outline" className="border-[#d7b45a]/40 text-[#fff7e7]">Explore the evidence</Button></Link></div>
          </div>
...
function EvidenceCard({ record, language }: { record: (typeof APPROVED_UNIVERSE_RECORDS)[number]; language: SupportedLanguage }) { return <aside aria-label="Evidence panel" className="rv-glass-card rounded-3xl p-6 sm:p-8"><p className="text-xs font-bold uppercase tracking-[0.18em] text-[#d7b45a]">Evidence panel</p><h3 className="mt-4 font-serif text-2xl font-bold text-white">{record.titleEn}</h3><dl className="mt-5 space-y-3 text-sm"><div><dt className="text-[#f4ead4]/45">Record ID</dt><dd className="font-mono text-[#f4ead4]/85">{record.id}</dd></div><div><dt className="text-[#f4ead4]/45">Locator</dt><dd className="text-[#f4ead4]/85">{record.locator}</dd></div><div><dt className="text-[#f4ead4]/45">Source IDs</dt><dd className="font-mono text-[#f4ead4]/85">{record.sourceIds.join(", ")}</dd></div></dl><Link href={localizedPath(language, `/sargas/${encodeURIComponent(record.id)}`)}><Button variant="outline" className="mt-6 border-[#d7b45a]/40 text-[#fff7e7]">Read source record</Button></Link></aside> }
